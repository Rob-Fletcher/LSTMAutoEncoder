...
from models.LSTMAEPartition import *
from loss.PartitionLoss import *
...
def train(args):
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), weight_decay=0.01, momentum=0.1, centered=True)
    # optimizer = torch.optim.SGD(model.parameters(), momentum=0.9)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer)
    # optimizer = RAdam(model.parameters())  # Rectified Adam optimizer
    print("+ Model Loaded.")
    ...
    part_loss = PartitionLoss(lmbda=999)
    ...
    for epoch in range(start_epoch,start_epoch+args.epochs):
        ...
        for batch_i, (paths, pred) in enumerate(dataloader):
            ...
            # Generate the weak labels. If 95% of the isParked column is 1 then set
            # the label for that sample to 1. This flags paths that are parked
            # for the entire path to 1.
            weaklabels = (paths[:,:,2].sum(dim=1)/paths.shape[1] > 0.95).float()
            weaklabels.requires_grad=False

            # Create the masking matrix C. This sets the first 3/4 of the
            # latent vector to 1 and the last 1/4 to 0.
            C = torch.ones((paths.shape[0], args.hidden_size))
            C[:,3*args.hidden_size//4:] = 0
            C.requires_grad=False

            # Run the model for the current batch and get loss
            model_s_time = time.time()
            paths = paths.to(device)
            recon, latent = model(paths[:,:,:2])
            recon_loss = mse_loss(recon, paths[:,:,:2])
            # print(weaklabels.shape, C.shape, latent[:,0,:].shape)
            p_loss = part_loss(weaklabels.view(-1,1), latent[:,0,:], C)
            total_loss = recon_loss + p_loss
        ...
        writer.add_scalar(tag="loss/partition loss", scalar_value=e_p_loss/float(len(dataloader)), global_step=int(epoch))
        ...
        print(f"               part loss:   {e_p_loss/float(len(dataloader))}")
